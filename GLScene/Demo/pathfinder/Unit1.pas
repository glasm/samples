...
uses
  ...
  Dialogs, GLObjects, GLGraph, GLScene, GLCoordinates, GLCadencer,
  GLWin32Viewer, GLCrossPlatform, BaseClasses, VectorGeometry, VectorTypes,
  GLMaterial, Spline;
...
begin
  ...
  for i := 0 to 250 do begin
    ...
    if ((j < px - 1) or (j > px + 1) or (k < py - 1) or (k > py + 1)) and
      ((j < tx - 1) or (j > tx + 1) or (k < ty - 1) or (k > ty + 1)) then begin

      with TGLCube.CreateAsChild( dc_walls ) do begin
      ...
      for m := -1 to 1 do
        for n := -1 to 1 do begin
          tarr[j*3 + m+1, k*3 + n+1][0] := j * 3000 + m * 1000;
          tarr[j*3 + m+1, k*3 + n+1][1] := k * 3000 + n * 1000;
          tarr[j*3 + m+1, k*3 + n+1][2] := 1000;
          tarr[j*3 + m+1, k*3 + n+1][3] := 0;
        end;
...
const
  r = 300; d0 = 1159; d1 = 1004; d2 = 580; // for player
  d: array[-2..13] of TVector2i = (
    (d2,-d1),(d1,-d2),(d0,0),(d1,d2),(d2,d1),(0,d0),(-d2,d1),(-d1,d2),
    (-d0,0),(-d1,-d2),(-d2,-d1),(0,-d0),(d2,-d1),(d1,-d2),(d0,0),(d1,d2)
  );
  l: array[0..4] of integer = (0,-1,1,-2,2); // rays order F,L30°,R30°,L60°,R60° 
var
    i,j,m: integer;
    sp0,sp1,st: integer;
    sph: TGLSphere;
  ...
  var j,k:integer;v:TVector4i;
  begin
    result := true;
    for j := cx - 1 to cx + 1 do
      for k := cy - 1 to cy + 1 do begin
        v := carr[j,k];
        if v[3] = 2 then begin
          drawpath;
          st := 0;
        ...
        if ((v[2]+r)*(v[2]+r)*0.9 > (v[0]-x)*(v[0]-x)+(v[1]-y)*(v[1]-y)) then begin
          result := false;
          exit;
        end;
      end;
  ...
  begin
    carr[cx,cy][0] := x;
    carr[cx,cy][1] := y;
    carr[cx,cy][2] := r; // marker radius
    carr[cx,cy][3] := t; // marker type
    ...
    for i := sp0 to sp1 do
      for j := 0 to 4 do
        with sp[m] do begin
          ...
          x := sp[i].x + d[sp[i].dir + l[j]][0];
          xc := round(3 * x / 3000 + 1);
          if (xc <= 0) or (xc > arrw*3 - 2) then continue;
          y := sp[i].y + d[sp[i].dir + l[j]][1];
          yc := round(3 * y / 3000 + 1);
          if (yc <= 0) or (yc > arrh*3 - 2) then continue;

          b := (carr[xc,yc][2]=0) and check(xc,yc,x,y);
...
begin
  ...
  if path.Nodes.Count > 0 then begin
