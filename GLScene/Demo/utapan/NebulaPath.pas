...
var
  ...
  //--------------------
	...
	walkability : array[0..100+1,0..100+1] of integer; //;array that holds wall/obstacle information
	openList : array[0..(100*100)+2] of integer;//;array holding open list items
	listArray : array[0..100+1,0..100+1] of integer;//  ;array used to record
	openX : array[0..100*100+2] of integer;// ;stores the x location of an item on the open list
	openY : array[0..100*100+2] of integer;// ;stores the y location of an item on the open list
	parentX : array[0..100+1,0..100+1] of integer;// ;array to store parent of each cell (x)
	parentY : array[0..100+1,0..100+1] of integer;//;array to store parent of each cell (y)
	Fcost : array[0..101*101] of integer;	 //	;array to store F cost for each cell.
	 Gcost: array [0..100+1,0..100+1] of integer; // 	;array to store G cost for each cell.
	 Hcost: array [0..100*100+2] of integer; //		;array to store H cost for each cell.
	 pathLength: array[0..1] of integer; //      ;length of the found path
	...
	tempWalkability: array [0..100+2,0..100+2] of integer; //
  ...
  procedure initFind;
  procedure drawmap(width:integer;height:integer);
  procedure drawmapblock(x:integer;y:integer;width:integer;height:integer;value:integer);

  Function findPath(mapWidth:integer;mapHeight:integer;startingX:integer;startingY:integer;targetX:integer;targetY:integer):integer;
  Function readPathX2(pathfinderID:integer;pathLocation:integer):integer;
  Function readPathY2(pathfinderID:integer;pathLocation:integer):integer;
...
for x1:= 0 to 100*100 do begin
  openlist[x1] :=0;
  fcost[x1] := 0;
end;

for x1:=0 to 101 do begin
  for y1:=0 to 101 do begin
    parentx[x1,y1] := 0;
    parenty[x1,y1] := 0;
    patharray[x1] := 0;
...
for x1 := 0 to 100*100+2 do begin
        ...
        begin
          ...
          begin
						If listArray[a,b] <> onClosedList then begin
							If walkability[a,b] <> unwalkable then begin
								If (walkability[a,b] <> deadend) then begin
									corner := walkable;
  ...
  begin
    ...
    pathY := targetY;
		...
		cellposition2 := pathlength[pathfinderID]*2+10;
		Repeat
	...
	findpath := path;//path;// Returns 1 if a path has been found, 2 if no path exists.
  exit;
	noPath:
