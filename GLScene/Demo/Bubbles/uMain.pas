...
uses
  ...
  GLCoordinates, GLCadencer, GLMaterial, AsyncTimer, VectorGeometry,
  VectorTypes, GLCompositeImage,
  ...
  begin
    for i := 0 to high(darr) do
      with vbo.garr[i * 4] do
        darr[i] := round(( v[0] - px ) * ( v[0] - px ) +
                         ( v[1] - py ) * ( v[1] - py ) +
                         ( v[2] - pz ) * ( v[2] - pz ));
    a2 := length(darr);
    a3 := a2 div 2;
    while a3 > 0 do begin
      for a1 := 0 to a2 - a3 - 1 do begin
        a4 := a1;
        while (a4 >= 0) and (darr[a4] < darr[a4 + a3]) do begin
          k := darr[a4]; darr[a4] := darr[a4 + a3]; darr[a4 + a3] := k;
          a5 := a4 * 4;
          a6 := a5 + a3 * 4;
          k := vbo.iarr[n, a5]; vbo.iarr[n, a5] := vbo.iarr[n, a6]; vbo.iarr[n, a6] := k;
          k := vbo.iarr[n, a5+1]; vbo.iarr[n, a5+1] := vbo.iarr[n, a6+1]; vbo.iarr[n, a6+1] := k;
          k := vbo.iarr[n, a5+2]; vbo.iarr[n, a5+2] := vbo.iarr[n, a6+2]; vbo.iarr[n, a6+2] := k;
          k := vbo.iarr[n, a5+3]; vbo.iarr[n, a5+3] := vbo.iarr[n, a6+3]; vbo.iarr[n, a6+3] := k;
          if a4 > a3 then a4 := a4 - a3
            else a4 := 0;
        end;
      end;
      a3 := a3 div 2;
