...
uses
  ...
  GLScene, VectorTypes, VectorGeometry, GLRenderContextInfo,
  OpenGLTokens, OpenGLAdapter, GLContext, GLUtils;
...
type
  ...
  public
    ...
    procedure DoRender( var a_Rci:TRenderContextInfo;
...
begin
  ...
  gl.BufferData( GL_ELEMENT_ARRAY_BUFFER, 4 * length(iarr[0]), nil, GL_DYNAMIC_DRAW);
...
begin
  ...
  if AShad <> '' then begin
    ...
    if not f_glsl.LinkProgram then raise Exception.Create( f_glsl.InfoLog );
    if not f_glsl.ValidateProgram then raise Exception.Create( f_glsl.InfoLog );
  end;
...
begin
  ...
  if not f_init then exit;

  Material.Apply(a_Rci);

  if f_glsl <> nil then begin
  ...
  if f_camLook <> f_oldCamLook then begin
    gl.BufferSubData( GL_ELEMENT_ARRAY_BUFFER, 0, length(iarr[0]) * 4, @iarr[f_camLook][0] );
