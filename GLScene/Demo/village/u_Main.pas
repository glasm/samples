...
uses
  ...
  AsyncTimer, GLMaterial, GLCadencer, GLWin32Viewer, GLCrossPlatform, Controls,
  BaseClasses, GLScene, GLCoordinates, GLObjects, GLVectorFileObjects,
  GLKeyboard, VectorGeometry, GLTexture;
...
const // material list (from 3DS)
    c:array[0..28] of string = (
  'd40839b9','hibara01','beton','hibara03','ed79fe48','dom002','dom003',
  'dom004','e1e075eb','30e5522d','domedv','dom005','dom009','dom008','stoun',
  'tavern','shipyard','domahati','pena','BackGrnd','viveski','plane',
  'tovpalat','scaly','tovar01','sunduki','osobnyk','statui01','63d9d8cd');
var
    m: TGLLibMaterial;
    i,j: integer;
begin
  ...
  log('"sky.dds"');
  with DDStex(matlib, 'sky', 'sky.dds') do begin
...
begin
  ...
  if (not iskeydown(vk_rbutton)) and (m_turn) then begin
    m_turn := false;
  ...
  // keyboard
  if iskeydown(vk_shift) then spd := 10 else spd := 5;

  if iskeydown(ord('W')) then f := 1
    else if iskeydown(ord('S')) then f := -1
      else f := 0;
  dc_cam.Position.Translate(vectorscale(cam.AbsoluteDirection, f * dt * spd));

  if iskeydown(ord('A')) then f := 1
    else if iskeydown(ord('D')) then f := -1
      else f := 0;
