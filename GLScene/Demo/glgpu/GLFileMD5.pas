...
var
  ...
  PrepareMD5forHWskinning: boolean;
...
procedure TGLMD5VectorFile.LoadFromStream(aStream : TStream);
  ...
  var
    ...
    blendedVert, transformedVert, tempVert : TAffineVector;
    vw: array[0..3]of TVector;
    i,j,k : Integer;
    mat : TMatrix;

    TexCoordHWSkin1,TexCoordHWSkin2,TexCoordHWSkin3,TexCoordHWSkin4:TVector4f;

    weightFract,temps: single;
  ...
  begin
    ...
    mesh.TexCoordsEx[1].Count:=numverts;
    mesh.TexCoordsEx[2].Count:=numverts;
    mesh.TexCoordsEx[3].Count:=numverts;
    mesh.TexCoordsEx[4].Count:=numverts;
    mesh.TexCoordsEx[5].Count:=numverts;
    mesh.TexCoordsEx[6].Count:=numverts;
    mesh.Colors.Count:=numverts;
    ...
    for i:=0 to numverts-1 do begin
      ...
      TexCoordHWSkin1:=vectorMake(0,0,0,0);
      TexCoordHWSkin2:=vectorMake(0,0,0,0);
      TexCoordHWSkin3:=vectorMake(0,0,0,0);
      TexCoordHWSkin4:=vectorMake(0,0,0,0);
      ...
      for j:=0 to mesh.BonesPerVertex-1 do begin
        if (j<VertexWeightCount[i]) and (j<4 {!!!} ) then begin
          k:=VertexWeightID[i]+j;

          tempBone:=VertexBoneRef[k];
          tempWeight:=VertexWeight[k];

          mesh.VerticesBonesWeights^[i]^[j].BoneID:=VertexBoneRef[k];
          mesh.VerticesBonesWeights^[i]^[j].Weight:=VertexWeight[k];

          //if not PrepareMD5forHWskinning then begin
           ...
           setVector( vw[j], VertexWeighted[k] );
           vw[j][3] := 1;
          //end else begin
           //AddVector(blendedVert, VectorScale(VertexWeighted[k],VertexWeight[k]));
          //end;
        ...
        //build "TexCoords" for HW skinning
        if j<4 then begin //firt 4 bones per vert

         TexCoordHWSkin1[j]:=tempBone;
         TexCoordHWSkin2[j]:=tempWeight;

        end;// else if j<8 then begin //next 4 bones per vert

        // TexCoordHWSkin3[j-4]:=tempBone;
        // TexCoordHWSkin4[j-4]:=tempWeight;
      ...
      //q := q + format('%.3f %.3f %.3f'#13#10,[blendedVert[0],blendedVert[1],blendedVert[2]]);

      if PrepareMD5forHWskinning then begin
       mesh.TexCoordsEx[1][i]:=TexCoordHWSkin1;
       mesh.TexCoordsEx[2][i]:=TexCoordHWSkin2;
       mesh.TexCoordsEx[3][i]:=vw[0]; {!!! VertexWeighted1}
       mesh.TexCoordsEx[4][i]:=vw[1]; {    VertexWeighted2}
       mesh.TexCoordsEx[5][i]:=vw[2]; {    VertexWeighted3}
       mesh.TexCoordsEx[6][i]:=vw[3]; {    VertexWeighted4}
      end;

    end;

    //SaveAnsiStringToFile( 'qwe' + inttostr(zzz), q );
    //inc(zzz);
