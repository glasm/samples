...
begin
  ...
  gl.BufferData(GL_ARRAY_BUFFER, 12 * length(FNArr), @FNArr[0][0], GL_STATIC_DRAW);

  gl.GenBuffers(1, @FBufs[2]);
  gl.BindBuffer(GL_ARRAY_BUFFER, FBufs[2]);
  gl.BufferData(GL_ARRAY_BUFFER, 12 * length(FTArr), @FTArr[0][0], GL_STATIC_DRAW);
...
procedure TBenchGPUInst.DoRender( var rci: TRenderContextInfo;
